        ...
        <div class="col-lg-12 col-md-12 col-12">
            ...
            <div class="border-bottom pb-4 mb-4 d-lg-flex align-items-center justify-content-between">
                <div class="mb-2 mb-lg-0">
                    <h1 class="mb-0 h2 fw-bold mb-3">جميع الخدمات </h1>
                    <!-- Breadcrumb -->
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">      جميع الخدمات ({{$transaction->count()}})    </li>
                        </ol>
                    </nav>
                </div>
                <div class="d-flex align-items-center">
                    <!-- button -->
                    <a href="{{url('AddTransactions')}}" class="btn btn-primary me-2"> <i class="fa-solid fa-plus"></i> إضافة خدمة  </a>
                                ...
                                <table id="dt-filter-search" class="table table-hover table-row-bordered  border rounded">
                                    ...
                                    @foreach ($transaction as $key => $transactions)
                                        <tr>
                                            ...
                                            <td><b><a style="color: #64748b;" TARGET="_blank" href="{{route('InfoTransactions',$transactions->slug)}}">{{$transactions->name}}</a> </b></td>
                                            <td> AED <b> {{number_format(  $transactions->ofees + $transactions->lfees , 2, ',', '.')}} </b> </td>
                                            <td> <i class="fe fe-map-pin text-muted me-2 fa-lg"></i> <b> {{$transactions->emirate}} </b> </td>
                                            <td> <i class="fa-brands fa-nfc-directional me-2 fa-lg"></i> <b> {{$transactions->authority}} </b> </td>

                                            <td>
                                                <a href="{{route('EditTransactions',$transactions->slug)}}" class=" text-muted text-primary-hover texttooltip" data-template="one">
                                                    <i class="fe fe-edit fs-3"  data-bs-toggle="offcanvas" data-bs-target="#offcanvasRightEdit" ></i>
                                                </a>
                                            </td>
                                            <td>
                                                <form method="POST" action="{{ route('DestroyTransaction', $transactions->id) }}">
                                                    @csrf
